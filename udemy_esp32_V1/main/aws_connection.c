...
void ledtActuate_Callback(const char *pJsonString, uint32_t JsonStringDataLen, jsonStruct_t *pContext)
{
    ...
    if(pContext != NULL)
    {
        ESP_LOGI(TAG, "Delta - blinds state changed to %d", *(int *) (pContext->pData));
        blind_state = *(int *) (pContext->pData);
    }
}

void aws_iot_task(void *param)
{
	int blinds_state_recived = 0;
	int blinds_state_send = 0;
	update_needed = false;
	data_recived = false;
	while (1)
	{
		...
		jsonStruct_t blinds_actuator = {
			.cb = ledtActuate_Callback,
			.pData = &blinds_state_recived,
			.pKey = "poz",
		...
		rc = aws_iot_shadow_register_delta(&mqttClient, &blinds_actuator);
		if(SUCCESS != rc)
		{
		...
		while(NETWORK_ATTEMPTING_RECONNECT == rc || NETWORK_RECONNECTED == rc || SUCCESS == rc)
		{
			...
			if(SUCCESS == rc)
			{
				rc = aws_iot_shadow_add_reported(JsonDocumentBuffer, sizeOfJsonDocumentBuffer, 1, &blinds_actuator);
				if(SUCCESS == rc)
				{
					...
					if(SUCCESS == rc && update_needed)
					{
						...
						data_recived = true;
						blinds_state_send = blinds_state_recived;
